...
int main(int argc, char *argv[])
{
    ...
    glutCreateWindow("OpenGL Model Drawing Function Examples");

    // 创建菜单并定义菜单回调函数
    int nGlutPolyMenu = glutCreateMenu(ProcessMenu);
    glutAddMenuEntry("Wireframe Tetrahedron", 1); // 创建GLUT多面体绘制菜单
    glutAddMenuEntry("Solid Tetrahedron", 2);
    glutAddMenuEntry("Wireframe Octahedron", 3);
    glutAddMenuEntry("Solid Octahedron", 4);

    int nGlutCurveMenu = glutCreateMenu(ProcessMenu); // 创建GLUT曲面绘制菜单
    glutAddMenuEntry("Wireframe Spherical", 5);
    glutAddMenuEntry("Solid Spherical", 6);
    ...
    glutAddMenuEntry("Wireframe Conical", 10);
    glutAddMenuEntry("Wireframe Circular", 11);

    int nMainMenu = glutCreateMenu(ProcessMenu); // 创建主菜单
    glutAddSubMenu("GLUT Polyhedra", nGlutPolyMenu);
